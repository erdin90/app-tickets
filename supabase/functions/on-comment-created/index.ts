...
serve(async (req) => {
  try {
    ...
    // admitir varias convenciones de secreto para evitar faltantes: SERVICE_ROLE | SUPABASE_SERVICE_ROLE | SUPABASE_SERVICE_ROLE_KEY
  ...
  const RESEND_FROM = Deno.env.get("RESEND_FROM") ?? "info@paqva.com";
    if (!SERVICE_ROLE) {
      console.error("Missing SERVICE_ROLE/SUPABASE_SERVICE_ROLE secret");
      return new Response("missing service role", { status: 500 });
    }
    // Worker URL es opcional si existe RESEND_API_KEY (usamos fallback)
    if (!SEND_EMAIL_WORKER_URL && !RESEND_API_KEY) {
      console.error("Missing SEND_EMAIL_WORKER_URL and RESEND_API_KEY secrets");
      return new Response("no email transport configured", { status: 500 });
    }
...
`;
    ...
    // --- envío: primero intentamos Worker, si falla y tenemos RESEND_API_KEY usamos Resend como fallback ---
    async function tryWorker() {
      if (!SEND_EMAIL_WORKER_URL) return { ok: false, msg: "no worker url" } as const;
      const r = await fetch(SEND_EMAIL_WORKER_URL, {
        method: "POST",
      ...
      console.error("send-email-worker error:", txt);
      return { ok: false as const, msg: txt };
    }

    async function tryResend() {
      if (!RESEND_API_KEY) return { ok: false, msg: "no resend key" } as const;
      ...
      if (rr.ok) return { ok: true as const };
    ...
    if (!w.ok) {
      ...
      if (!r.ok) {
        return new Response(`email error: ${w.msg || r.msg || 'unknown'}`, { status: 500 });
      }
    }
