...
export async function listTickets(params: ListParams) {
  const { group, priority, assignedTo, business, due='all', q, page=1, pageSize=10 } = params ?? {};
 ...
 */
export async function countTicketsByCategory(params?: { assignedTo?: string; business?: string; priority?: TicketPriority }) {
  const { assignedTo, business, priority } = params ?? {};
