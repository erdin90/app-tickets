...
export async function GET(request: Request) {
  ...
  type ProfileRow = { id: string; full_name: string | null; avatar_url: string | null; role: string | null; ext: string | null };
  const profileMap = new Map<string, ProfileRow>();
  if (ids.length) {
    const { data: profiles, error: profileError } = await supabase
      .from('profiles')
      .select('id, full_name, avatar_url, role, ext')
      .in('id', ids);
...
export async function POST(request: Request) {
  ...
  const { email, password, full_name, role, avatar_url, ext } = body as Record<string, string | undefined>;
