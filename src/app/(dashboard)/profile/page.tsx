...
function ProfileContent() {
  ...
  async function saveName() {
    ...
    const { error } = await supabase
      .from('profiles')
      .update({ full_name: name, ext: cleanedExt })
      .eq('id', profile.id);
    setSaving(false);
    if (error) {
    ...
    setProfile({ ...profile, full_name: name, ext: cleanedExt });
  ...
  const roleLabel =
    profile?.role === 'manager'
      ? (t('role.manager') as string)
      : profile?.role
      ? (t('role.technician') as string)
      : undefined;
  ...
  const dirty = dirtyName || dirtyExt;
    ...
    <>
      ...
      <div className="container" style={{ display: 'grid', gap: 16 }}>
        ...
        <div className="ticket" style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr auto', alignItems: 'center', gap: 16 }}>
          ...
          <div style={{ justifySelf: isMobile ? 'end' : 'auto' }}>
              ...
              <div className="section-title" style={{ margin: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                {!editingInfo ? (
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button className="btn" onClick={() => { setName(profile.full_name ?? ''); setExt(profile.ext ?? ''); setEditingInfo(false); }} style={{ height: 28, padding: '0 10px', fontSize: 12 }}>Cancelar</button>
