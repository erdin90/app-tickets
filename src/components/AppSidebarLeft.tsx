...
type UserProfile = { full_name?: string; role?: string; email?: string };
    ...
    const items: NavItem[] = [
      {
        ...
        icon: '🎫',
        label: 'Tickets',
       // children: isManagerRole ? [{ href: '/tickets/board', label: 'Tablero' }] : undefined,
      },
    ...
    <div className="side-inner">
      ...
      <div className="side-footer">
        <div className="user-card">
          <Link href="/profile" className="avatar" title="Perfil" onClick={onClose}>👤</Link>
          <div className="user-meta">
            <div className="user-name">{user?.full_name || '—'}</div>
            <div className="user-role">{user?.role || '—'}</div>
          </div>
          <button className="btn btn-icon exit-btn" title="Cerrar sesión" aria-label="Cerrar sesión" onClick={signOut}>⎋</button>
          </div>
      </div>
    </div>
  );
